...
class YawListener(Node):
    """
    A ROS2 node that listens to 7 different topics: AMCL, Odom, and 5 SML yaw topics 
    (one for each student), records yaw data, and generates a single graph with all values.

    Attributes:
    ----------
    yaw_data : list
        Stores yaw data over time from the 7 topics.
    start_time : float
    ...
    def __init__(self):
        ...
        self.subscription_sml_yaw_charlize = self.create_subscription(
            Float64,
            '/sml_yaw_charlize',  # Replace with the actual topic name
            self.yaw_callback_charlize,
            10)
    ...
    def yaw_callback_charlize(self, msg):
        """Callback for Charlize's SML yaw topic."""
        self.yaw_charlize = msg.data
        self.record_yaw()
    ...
    def record_yaw(self):
        ...
        """
        if (self.yaw_jarred is not None and self.yaw_charlize is not None and 
            self.yaw_sohail is not None and self.yaw_chidalu is not None and 
            self.yaw_tejas is not None and self.yaw_amcl is not None and 
            self.yaw_odom is not None):

            elapsed_time = time.time() - self.start_time

            # Record all yaw values to CSV
            self.yaw_data.append((elapsed_time, self.yaw_jarred, self.yaw_charlize, self.yaw_sohail,
                                  self.yaw_chidalu, self.yaw_tejas, self.yaw_amcl, self.yaw_odom))
            self.csv_writer.writerow([elapsed_time, self.yaw_jarred, self.yaw_charlize, self.yaw_sohail,
                                      self.yaw_chidalu, self.yaw_tejas, self.yaw_amcl, self.yaw_odom])
    ...
    def plot_yaw_data(self):
        ...
        if len(self.yaw_data) > 0:
            times, yaw_jarred, yaw_charlize, yaw_sohail, yaw_chidalu, yaw_tejas, yaw_amcl, yaw_odom = zip(*self.yaw_data)

            # Plot all seven yaw values on one plot
            plt.figure()
            plt.plot(times, yaw_jarred, label='Jarred Deluca - SML Yaw', color='blue')
            plt.plot(times, yaw_charlize, label='Charlize Petersen - SML Yaw', color='green')
            ...
            plt.legend()
            plt.grid(True)
            plt.savefig(os.path.join(self.output_dir, 'all_yaw_values.png'))
            plt.close()
